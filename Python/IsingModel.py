...
1 2 3 4 5 6 7 8
 ...
 But I learned a nice thing from my desire to optimize code speed.
 It is the usage of Numba’s JIT compiler. read more about that here [link](https://melashri.net/url/b).
 I also instead of using multiple nested loops I dragged all these into just one.
 Imagine running 50x50 lattice simulation in my older codes for hours (one took 6 hours)
 vs 15 minutes for the currect script. (On my Mac m1 Machine).
 also I made the code avilable on colab and can be accssed here (without much comments)
 [link](https://melashri.net/url/c).
"""
...
# from tqdm.notebook import tqdm # use this if working with jupyter notebook to avoid printing a new line
# for each iteration
from rich.progress import \
    track  # we can use amazing rich library to do the same (more interesting output) change "tqdm" later to "track"
